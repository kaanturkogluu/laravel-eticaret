...
class MokaposService extends BasePaymentService
{
    public function initiatePayment(Order $order, array $paymentData = []): array
    {
        try {
            ...
            $apiUrl = $this->getApiUrl('api/v1/payments');
            ...
            $requestData = [
                ...
                'webhook_url' => $this->getConfig('webhook_url')
            ];
            ...
            if ($response['success'] && isset($response['data']['payment_url'])) {
                ...
                ], 'Mokapos ödeme işlemi başlatıldı');
            }
            ...
            return $this->errorResponse('Mokapos ödeme işlemi başlatılamadı', $response);

        } catch (\Exception $e) {
            return $this->errorResponse('Mokapos ödeme işlemi sırasında hata oluştu: ' . $e->getMessage());
        }
    }
    ...
    {
        try {
            $apiUrl = $this->getApiUrl("api/v1/payments/{$transaction->external_transaction_id}");
            $apiKey = $this->getConfig('api_key');
            ...
            return $this->errorResponse('Mokapos durum kontrolü başarısız', $response);

        } catch (\Exception $e) {
            return $this->errorResponse('Mokapos durum kontrolü sırasında hata oluştu: ' . $e->getMessage());
        }
    }
    ...
    public function handleCallback(Request $request): array
    {
        try {
            ...
            return $this->errorResponse('Mokapos callback işlemi sırasında hata oluştu: ' . $e->getMessage());
        }
    }
    ...
    public function refund(PaymentTransaction $transaction, float $amount = null): array
    {
        try {
            ...
            $apiUrl = $this->getApiUrl("api/v1/payments/{$transaction->external_transaction_id}/refund");
            $apiKey = $this->getConfig('api_key');
            ...
            if ($response['success']) {
                $transaction->refund($refundAmount, 'Mokapos iade işlemi');
                return $this->successResponse(['refund_amount' => $refundAmount], 'Mokapos iade işlemi başarılı');
            }

            return $this->errorResponse('Mokapos iade işlemi başarısız', $response);

        } catch (\Exception $e) {
            return $this->errorResponse('Mokapos iade işlemi sırasında hata oluştu: ' . $e->getMessage());
        }
    }
