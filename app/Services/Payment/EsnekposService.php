...
class EsnekposService extends BasePaymentService
{
    public function initiatePayment(Order $order, array $paymentData = []): array
    {
        try {
            ...
            if ($response['success'] && isset($response['data']['payment_url'])) {
                ...
                ], 'Esnekpos ödeme işlemi başlatıldı');
            }
            ...
            return $this->errorResponse('Esnekpos ödeme işlemi başlatılamadı', $response);

        } catch (\Exception $e) {
            return $this->errorResponse('Esnekpos ödeme işlemi sırasında hata oluştu: ' . $e->getMessage());
        }
    }
    ...
    public function checkPaymentStatus(PaymentTransaction $transaction): array
    {
        try {
            ...
            return $this->errorResponse('Esnekpos durum kontrolü başarısız', $response);

        } catch (\Exception $e) {
            return $this->errorResponse('Esnekpos durum kontrolü sırasında hata oluştu: ' . $e->getMessage());
        }
    }
    ...
    public function handleCallback(Request $request): array
    {
        try {
            ...
            return $this->errorResponse('Esnekpos callback işlemi sırasında hata oluştu: ' . $e->getMessage());
        }
    }
    ...
    public function refund(PaymentTransaction $transaction, float $amount = null): array
    {
        try {
            ...
            if ($response['success']) {
                $transaction->refund($refundAmount, 'Esnekpos iade işlemi');
                return $this->successResponse(['refund_amount' => $refundAmount], 'Esnekpos iade işlemi başarılı');
            }

            return $this->errorResponse('Esnekpos iade işlemi başarısız', $response);

        } catch (\Exception $e) {
            return $this->errorResponse('Esnekpos iade işlemi sırasında hata oluştu: ' . $e->getMessage());
        }
    }
