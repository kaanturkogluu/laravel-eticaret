...
class OdealService extends BasePaymentService
{
    public function initiatePayment(Order $order, array $paymentData = []): array
    {
        try {
            ...
            return $this->errorResponse('Ödeal ödeme işlemi başlatılamadı', $response);

        } catch (\Exception $e) {
            return $this->errorResponse('Ödeal ödeme işlemi sırasında hata oluştu: ' . $e->getMessage());
        }
    }
    ...
    public function checkPaymentStatus(PaymentTransaction $transaction): array
    {
        try {
            ...
            return $this->errorResponse('Ödeal durum kontrolü başarısız', $response);

        } catch (\Exception $e) {
            return $this->errorResponse('Ödeal durum kontrolü sırasında hata oluştu: ' . $e->getMessage());
        }
    }
    ...
    public function handleCallback(Request $request): array
    {
        try {
            ...
            return $this->errorResponse('Ödeal callback işlemi sırasında hata oluştu: ' . $e->getMessage());
        }
    }
    ...
    public function refund(PaymentTransaction $transaction, float $amount = null): array
    {
        try {
            ...
            if ($response['success']) {
                $transaction->refund($refundAmount, 'Ödeal iade işlemi');
                return $this->successResponse(['refund_amount' => $refundAmount], 'Ödeal iade işlemi başarılı');
            }

            return $this->errorResponse('Ödeal iade işlemi başarısız', $response);

        } catch (\Exception $e) {
            return $this->errorResponse('Ödeal iade işlemi sırasında hata oluştu: ' . $e->getMessage());
        }
    }
