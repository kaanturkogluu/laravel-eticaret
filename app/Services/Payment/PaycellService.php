...
class PaycellService extends BasePaymentService
{
    public function initiatePayment(Order $order, array $paymentData = []): array
    {
        try {
            ...
            if ($response['success'] && isset($response['data']['payment_url'])) {
                ...
                ], 'Paycell ödeme işlemi başlatıldı');
            }
            ...
            return $this->errorResponse('Paycell ödeme işlemi başlatılamadı', $response);

        } catch (\Exception $e) {
            return $this->errorResponse('Paycell ödeme işlemi sırasında hata oluştu: ' . $e->getMessage());
        }
    }
    ...
    public function checkPaymentStatus(PaymentTransaction $transaction): array
    {
        try {
            ...
            return $this->errorResponse('Paycell durum kontrolü başarısız', $response);

        } catch (\Exception $e) {
            return $this->errorResponse('Paycell durum kontrolü sırasında hata oluştu: ' . $e->getMessage());
        }
    }
    ...
    public function handleCallback(Request $request): array
    {
        try {
            ...
            return $this->errorResponse('Paycell callback işlemi sırasında hata oluştu: ' . $e->getMessage());
        }
    }
    ...
    public function refund(PaymentTransaction $transaction, float $amount = null): array
    {
        try {
            ...
            if ($response['success']) {
                $transaction->refund($refundAmount, 'Paycell iade işlemi');
                return $this->successResponse(['refund_amount' => $refundAmount], 'Paycell iade işlemi başarılı');
            }

            return $this->errorResponse('Paycell iade işlemi başarısız', $response);

        } catch (\Exception $e) {
            return $this->errorResponse('Paycell iade işlemi sırasında hata oluştu: ' . $e->getMessage());
        }
    }
