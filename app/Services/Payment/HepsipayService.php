...
class HepsipayService extends BasePaymentService
{
    public function initiatePayment(Order $order, array $paymentData = []): array
    {
        try {
            ...
            if ($response['success'] && isset($response['data']['payment_url'])) {
                ...
                ], 'Hepsipay ödeme işlemi başlatıldı');
            }
            ...
            return $this->errorResponse('Hepsipay ödeme işlemi başlatılamadı', $response);

        } catch (\Exception $e) {
            return $this->errorResponse('Hepsipay ödeme işlemi sırasında hata oluştu: ' . $e->getMessage());
        }
    }
    ...
    public function checkPaymentStatus(PaymentTransaction $transaction): array
    {
        try {
            ...
            return $this->errorResponse('Hepsipay durum kontrolü başarısız', $response);

        } catch (\Exception $e) {
            return $this->errorResponse('Hepsipay durum kontrolü sırasında hata oluştu: ' . $e->getMessage());
        }
    }
    ...
    public function handleCallback(Request $request): array
    {
        try {
            ...
            return $this->errorResponse('Hepsipay callback işlemi sırasında hata oluştu: ' . $e->getMessage());
        }
    }
    ...
    public function refund(PaymentTransaction $transaction, float $amount = null): array
    {
        try {
            ...
            if ($response['success']) {
                $transaction->refund($refundAmount, 'Hepsipay iade işlemi');
                return $this->successResponse(['refund_amount' => $refundAmount], 'Hepsipay iade işlemi başarılı');
            }

            return $this->errorResponse('Hepsipay iade işlemi başarısız', $response);

        } catch (\Exception $e) {
            return $this->errorResponse('Hepsipay iade işlemi sırasında hata oluştu: ' . $e->getMessage());
        }
    }
