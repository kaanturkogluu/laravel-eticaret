...
class PaparaService extends BasePaymentService
{
    public function initiatePayment(Order $order, array $paymentData = []): array
    {
        try {
            ...
            if ($response['success'] && isset($response['data']['payment_url'])) {
                ...
                ], 'Papara ödeme işlemi başlatıldı');
            }
            ...
            return $this->errorResponse('Papara ödeme işlemi başlatılamadı', $response);

        } catch (\Exception $e) {
            return $this->errorResponse('Papara ödeme işlemi sırasında hata oluştu: ' . $e->getMessage());
        }
    }
    ...
    public function checkPaymentStatus(PaymentTransaction $transaction): array
    {
        try {
            ...
            return $this->errorResponse('Papara durum kontrolü başarısız', $response);

        } catch (\Exception $e) {
            return $this->errorResponse('Papara durum kontrolü sırasında hata oluştu: ' . $e->getMessage());
        }
    }
    ...
    public function handleCallback(Request $request): array
    {
        try {
            ...
            return $this->errorResponse('Papara callback işlemi sırasında hata oluştu: ' . $e->getMessage());
        }
    }
    ...
    public function refund(PaymentTransaction $transaction, float $amount = null): array
    {
        try {
            ...
            if ($response['success']) {
                $transaction->refund($refundAmount, 'Papara iade işlemi');
                return $this->successResponse(['refund_amount' => $refundAmount], 'Papara iade işlemi başarılı');
            }

            return $this->errorResponse('Papara iade işlemi başarısız', $response);

        } catch (\Exception $e) {
            return $this->errorResponse('Papara iade işlemi sırasında hata oluştu: ' . $e->getMessage());
        }
    }
